...
- Brain Map、ドキュメント、画像取り込み、左右に並んだインスペクタービューを備えたローカルの Tauri デモアプリを実行する
...
デモは、API キーなしでルール専用モードで動作します。LLM を利用した Brain Map、より豊かな抽出、または画像文字起こしを行うには、アプリの設定でプロバイダーキーを追加するか、互換性のあるローカルモデルサーバーを実行します。
...
`--features llm` を指定してビルドした場合、画像ファイルもアクティブなプロバイダーを介して文字起こしされ、インデックス登録前に構造化された markdown に変換されます。
...
1. `--features llm` とアクティブなプロバイダー：画像ファイルはプロバイダーのビジョンパスを介して markdown に文字起こしされ、他のパースされたドキュメントと同様にインデックス登録されます。
2. `--features ocr`：ビジョン対応のプロバイダーを使用しない環境向けに、ローカルの Tesseract フォールバックを提供します。

ローカルの Tesseract OCR を有効化するには以下を使用します。

```bash
cargo build -p axonmind_engine --features ocr
```

プロバイダーによる文字起こしを優先し、ローカル OCR をフォールバックとして使用する場合は、両方のパスを有効にしてビルドします。

```bash
cargo build -p axonmind_engine --features "llm ocr"
```

アクティブな LLM プロバイダーがなく、かつ `ocr` 機能も有効でない状態で画像の取り込みを試行した場合、AxonMind は暗黙的に空のドキュメントを作成するのではなく、明確なエラーを返します。

すべてのプロバイダーアダプターが画像の文字起こしを公開しているわけではありません。設定されたプロバイダーがそのパスでの画像 OCR をサポートしていないと報告した場合は、ビジョン対応のプロバイダーを使用するか、ローカルの `ocr` フォールバックを有効にしてください。

Tauri インスペクターは、他のバイナリ形式と同様に、処理された画像のパースされた markdown/テキストを表示します。すでにインデックス登録されているファイルについては、キャッシュされた pageindex セクションを優先し、キャッシュされたセクションがまだ存在しない場合はプレビューパースにフォールバックします。
...
| インジェスト | Markdown、テキスト、PDF, DOCX、スプレッドシート、HTML、およびオプションのOCR/文字起こし付き画像ファイル |
| 抽出 | デフォルトで決定論的ルール。オプションの LLM 抽出と画像文字起こし |
...
| デモ | Brain Map、ドキュメントリスト、グラフの差分比較モーダル、画像取り込み、左右に並んだインスペクター、設定を備えたローカルの Tauri アプリ |
...
Processed Files（処理済みファイル）ビューで：すべてのドキュメントを選択 → Regenerate selected（選択項目を再生成）。これにより、すでに保存されている blob から読み取られ（再アップロードは不要）、ファイルが再パースされ、セクションツリーが再構築されて保存されます。AI プロバイダーが接続されていない場合、テキストベースのドキュメントは依然として高速でルール専用のままになります。画像ファイルには、アクティブな LLM プロバイダーか、`--features ocr` を指定したビルドのいずれかが必要になります。
...
テキストベースのドキュメントの場合、セクションツリーは、パースされたドキュメント構造のみから構築されます。`pageindex_enrich = true`（デフォルトは false）に設定されていない限り、LLM の抽出は関与しません。したがって、AI プロバイダーなしで既存のテキストファイルを再取り込みすることは非常に低コストです。blob からパース → 見出しツリーの構築 → SQLite FTS への書き込みを行います。画像ファイルは例外で、パースされた構造が存在する前にプロバイダーの文字起こしまたは OCR が必要になります。グラフのノードとエッジも再 upsert されますが、これは軽量です（すでに存在するため、基本的には no-op です）。
