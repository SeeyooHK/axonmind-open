...
- **組み込み可能なアーキテクチャ。** Rust エンジンを直接使用するか、CLI を呼び出すか、または TypeScript 転送インターフェースを介して React/Tauri UI を接続します。
...
- SQLite FTS5 を使用してグラフ全体を検索する
- グラフの状態を JSON としてエクスポートまたはインポートする
- 独自の製品 UI の背後にエンジンを組み込む
- Brain Map、ドキュメント、インスペクタービューを備えたローカルの Tauri デモアプリを実行する
