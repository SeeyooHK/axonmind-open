...
- **임베디드 아키텍처.** Rust 엔진을 직접 사용하거나, CLI를 호출하거나, TypeScript 전송 인터페이스를 통해 React/Tauri UI를 연결하세요.
...
- SQLite FTS5를 사용하여 그래프 전체를 검색합니다.
- 그래프 상태를 JSON으로 내보내거나 가져옵니다.
- 자체 제품 UI 뒤에 엔진을 임베드합니다.
- Brain Map, 문서 목록 및 검사기 뷰를 제공하는 로컬 Tauri 데모 앱을 실행합니다.
...
| 비즈니스 쿼리 | 특정 KPI 집중 탐색, 그래프 검색, 증거 찾기, 영향 반경 계산, 결정 추적, 추천 액션 제안 |
...
| 데모 환경 | Brain Map 시각화 화면, 문서 관리 목록, 속성 검사기 및 설정 화면을 탑재한 로컬 Tauri 앱 |
