...
- Brain Map, 문서 목록, 이미지 수집 및 좌우 배치형 검사기 뷰를 제공하는 로컬 Tauri 데모 앱을 실행합니다.
...
데모 앱은 API 키가 없는 경우 규칙 전용 모드로 작동합니다. LLM 기반의 Brain Map, 더 다채로운 관계 추출 또는 이미지 텍스트 변환(transcription)을 시험하려면 앱 설정에서 제공업체 API 키를 등록하거나 호환되는 로컬 LLM 서버를 구동하십시오.
...
`--features llm` 옵션으로 빌드하면, 이미지 파일도 활성화된 프로바이더를 통해 텍스트로 변환되어 인덱싱 전에 구조화된 마크다운으로 변환됩니다.
...
1. `--features llm` 및 활성화된 프로바이더: 이미지 파일은 프로바이더의 비전(vision) 경로를 통해 마크다운으로 텍스트 변환된 후, 다른 파싱된 문서와 마찬가지로 인덱싱됩니다.
2. `--features ocr`: 비전 기능이 있는 프로바이더가 없는 환경에서 로컬 Tesseract를 대체 기능(fallback)으로 사용합니다.

로컬 Tesseract OCR 활성화 명령:

```bash
cargo build -p axonmind_engine --features ocr
```

프로바이더를 통한 텍스트 변환을 우선적으로 시도하고 실패 시 로컬 OCR을 대체 기능으로 사용하려면 두 경로를 모두 활성화하여 빌드하십시오.

```bash
cargo build -p axonmind_engine --features "llm ocr"
```

활성화된 LLM 프로바이더나 `ocr` 기능이 모두 없는 상태에서 이미지 수집을 시도하면 AxonMind는 자동으로 빈 문서를 생성하는 대신 명확한 오류를 반환합니다.

모든 프로바이더 어댑터가 이미지 텍스트 변환을 노출하지는 않습니다. 설정된 프로바이더가 해당 경로에서 이미지 OCR을 지원하지 않는다고 보고하는 경우, 비전 지원 프로바이더를 사용하거나 로컬 `ocr` 대체 기능을 활성화하십시오.

Tauri 검사기는 다른 바이너리 형식과 마찬가지로 처리된 이미지에 대해 파싱된 마크다운/텍스트를 보여줍니다. 이미 인덱싱된 파일의 경우 캐시된 pageindex 섹션을 선호하며, 캐시된 섹션이 아직 없는 경우 미리보기 파싱으로 대체 동작합니다.
...
| 문서 수집/파싱 | 마크다운, 일반 텍스트, PDF, DOCX, 스프레드시트, HTML 파싱 및 선택적 OCR/텍스트 변환이 포함된 이미지 파일 |
| 관계 추출 | 기본적 규칙 감지 엔진 내장 및 선택적 LLM 관계 추출 및 이미지 텍스트 변환 지원 |
...
| 데모 환경 | Brain Map 시각화 화면, 문서 관리 목록, 그래프 비교 모달, 이미지 수집, 좌우 배치형 속성 검사기 및 설정 화면을 탑재한 로컬 Tauri 앱 |
...
Processed Files (처리 완료된 파일) 화면 진입 후: 전체 문서 선택 → Regenerate selected (선택된 파일 재생성) 버튼을 클릭하십시오. 이렇게 하면 이미 저장소에 보관된 파일 blob을 재사용(다시 업로드할 필요 없음)하여 파일을 분석하고, 섹션 구조를 빌드한 후 데이터베이스에 씁니다. 만약 인공지능(AI) 제공업체가 꺼져 있다면, 텍스트 기반 문서는 규칙 전용 모드로 빠르게 작동하며, 이미지 파일은 활성화된 LLM 프로바이더 또는 `--features ocr`이 포함된 빌드가 필요합니다.
...
텍스트 기반 문서의 경우, 섹션 문서 구조 트리는 오직 파일 파싱 구조 자체에만 의존하여 생성됩니다. 즉, `pageindex_enrich = true` (기본값 false) 옵션을 수동으로 켜지 않는 한 LLM의 도움을 받지 않습니다. 따라서 AI API를 연결하지 않고 단순히 기존 텍스트 문서를 재분석하는 작업은 매우 저렴합니다 (Blob 읽기 → 헤더 계층 구조 트리 빌드 → SQLite FTS 테이블 작성). 이미지 파일은 예외로, 파싱된 구조가 존재하기 전에 프로바이더 텍스트 변환 또는 OCR이 필요합니다. 이때 그래프 상의 노드와 에지들도 데이터베이스에 다시 기록(upsert)되지만, 대부분은 이미 존재하는 정보여서 변경 없이 빠르게 넘어갑니다.
