...
fn preview_chars(text: &str, max_chars: usize) -> &str {
        ...
        if log::log_enabled!(log::Level::Trace) {
            for item in &items {
                log::trace!(
                    ...
                    item.font_size,
                    item.font,
                    // Keep trace previews char-safe: some PDFs recover UTF-8 text with
                    // multibyte characters, so byte slicing here can panic on a boundary.
                    if item.text.len() > 80 {
                        preview_chars(&item.text, 80)
                    } else {
                        &item.text
                    }
                );
