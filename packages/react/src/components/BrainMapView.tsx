...
import type { AxonGraphElements, AxonGraphNode } from "../graph/adapter";
...
const THEME = {
  bg: "var(--axonmind-bg, #0b1120)",
  panel: "var(--axonmind-panel-bg, rgba(15,23,42,0.92))",
  panelStrong: "var(--axonmind-panel-strong-bg, rgba(15,23,42,0.95))",
  panelBorder: "var(--axonmind-panel-border, #334155)",
  line: "var(--axonmind-line, #1e293b)",
  text: "var(--axonmind-text, #e2e8f0)",
  textStrong: "var(--axonmind-text-strong, #f1f5f9)",
  textSoft: "var(--axonmind-text-soft, #cbd5e1)",
  textMuted: "var(--axonmind-text-muted, #94a3b8)",
  textSubtle: "var(--axonmind-text-subtle, #64748b)",
  surface: "var(--axonmind-surface, #020617)",
  inputBg: "var(--axonmind-input-bg, #0f172a)",
  danger: "var(--axonmind-danger, #f87171)",
};
...
export function BrainMapView({ elements, initialSummary, scopeDocIds, onSelectNode, onGoHome, style }: Props) {
  ...
  async function generate() {
    ...
    try {
      ...
      if (docIds && docIds.length > 0) {
        const suggested = await transport.suggestSummary(docIds);
        const resolved = await loadSummaryResolution(docIds);
        setSummary(resolved ? summaryFromResolution(resolved, suggested.labels) : (suggested as Summary));
      } else {
        const [snap, resolved] = await Promise.all([
          transport.getBrainMapDefaultConfig(),
          transport.resolveBrainMapDefaultSummary(),
        ]);
        setConfigSnapshot(snap as unknown as SummaryConfigSnapshot);
        setSummaryResolution(resolved as unknown as SummaryResolution);
        setSummary(summaryFromResolution(resolved as unknown as SummaryResolution));
      }
      setActiveCat(null);
      setLensPath([]);
      setChildLensResolutions([]);
  ...
  async function loadSummaryResolution(docIds?: string[]): Promise<SummaryResolution | null> {
    try {
      const res = await transport.resolveBrainMapDefaultSummary(
        docIds && docIds.length ? docIds : undefined,
      );
      const typed = res as unknown as SummaryResolution;
      setSummaryResolution(typed);
      return typed;
    } catch {
  ...
  async function loadLensChildren(parentLensId: string) {
    try {
      const res = await transport.resolveBrainMapLensChildren(parentLensId);
      setChildLensResolutions(res as unknown as LensResolution[]);
    } catch {
      setChildLensResolutions([]);
    }
  }
  ...
  async function loadConfigSnapshot() {
    const snap = await transport.getBrainMapDefaultConfig() as unknown as SummaryConfigSnapshot;
  ...
  async function saveEditor() {
    ...
    setError(null);
    try {
      await transport.updateBrainMapDefaultConfig({
  ...
  async function restoreDefaults() {
    ...
    setError(null);
    try {
      await transport.restoreBrainMapDefaultConfig();
      await generate();
      await loadConfigSnapshot();
      setEditOpen(false);
    } catch (e) {
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", width: "100%", height: "100%", background: THEME.bg, overflow: "hidden", ...style }}>
      {/* connector lines */}
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
        {circles.map(c => (
          <line key={c.id} x1={cx} y1={cy} x2={c.x} y2={c.y} stroke={THEME.line} strokeWidth={2} />
        ))}
      </svg>
      ...
      {summary && size.w > 0 && (
        <CircleNode
          ...
          bg={THEME.line} border={THEME.panelBorder} bold
          onClick={hubOnClick}
        />
      )}
        ...
        <div style={{
          ...
          {lensPath.map((lens, i) => (
            <React.Fragment key={lens.lens_id}>
              ...
              {i === lensPath.length - 1 ? (
                <span style={{ color: THEME.textSoft, fontSize: 11 }}>{lens.label}</span>
              ) : (
                <button
          ...
          {lensPath.length === 0 && activeCatLabel && (
            <>
              <span style={{ color: THEME.textSubtle, fontSize: 11 }}>›</span>
              <span style={{ color: THEME.textSoft, fontSize: 11 }}>{activeCatLabel}</span>
            </>
          )}
        </div>
      )}
      ...
      {activeCat === null && lensPath.length === 0 && summary && summaryResolution && summaryResolution.lenses.length > 0 && (
        <div style={{
          ...
          <div style={{ color: THEME.textSoft, fontSize: 12, fontWeight: 700, marginBottom: 8 }}>
            Lens Status
          </div>
          {summaryResolution.lenses.map((lens, i) => (
            <div key={lens.lens_id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ color: THEME.text, fontSize: 12, fontWeight: 600 }}>
              ...
              {lens.measure.explanation && (
                <div style={{ color: THEME.textSubtle, fontSize: 10, marginTop: 4, lineHeight: 1.35 }}>
                  {lens.measure.explanation}
                </div>
              )}
              {lens.health.explanation && (
                <div style={{ color: THEME.textSubtle, fontSize: 10, marginTop: 3, lineHeight: 1.35 }}>
                  {lens.health.explanation}
                </div>
              )}
            </div>
      ...
      {focusedLensResolution && (
        <div style={{
          ...
          <div style={{ color: THEME.text, fontSize: 12, fontWeight: 600, marginBottom: 6 }}>
            {focusedLensResolution.label}
          </div>
          <div style={{ color: THEME.textMuted, fontSize: 11, marginBottom: 6 }}>
            {topLevelSubLabel(focusedLensResolution)}
          </div>
          <div style={{ color: THEME.textSubtle, fontSize: 10, marginBottom: 8 }}>
          ...
          <div style={{ marginTop: 8 }}>
            <div style={{ color: THEME.textMuted, fontSize: 10, marginBottom: 4 }}>Supporting Nodes</div>
            <div style={{
              ...
              {focusedLensResolution.measure.supporting_nodes.length === 0 && (
                <div style={{ color: THEME.textSubtle, fontSize: 10 }}>none</div>
              )}
              {focusedLensResolution.measure.supporting_nodes.map((n, i) => {
                ...
                return (
                  <div key={`${n.node_id}:${i}`} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ color: THEME.textSubtle, fontSize: 10 }}>{n.kind} · {n.role}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ marginTop: 8 }}>
            <div style={{ color: THEME.textMuted, fontSize: 10, marginBottom: 4 }}>Evidence Trail (Interactive)</div>
            <div style={{
              background: THEME.surface,
              border: `1px solid ${THEME.line}`,
              borderRadius: 6,
              ...
              {focusedLensResolution.measure.evidence_lineage.map((item) => {
                ...
                return (
                  <div key={item.evidence_id} style={{ borderTop: `1px solid ${THEME.line}`, paddingTop: 6 }}>
                    <div style={{ color: THEME.text, fontSize: 11, fontWeight: 600 }}>
                      {item.source_node_name} ({item.source_type})
                    </div>
                    <div style={{ color: THEME.textSubtle, fontSize: 10, marginTop: 2 }}>id: {item.evidence_id}</div>
                    {item.row_ref && <div style={{ color: THEME.textSubtle, fontSize: 10 }}>row_ref: {item.row_ref}</div>}
                    {item.source_path && <div style={{ color: THEME.textSubtle, fontSize: 10, wordBreak: "break-word" }}>source_path: {item.source_path}</div>}
                    {item.quote && <div style={{ color: THEME.textMuted, fontSize: 10, marginTop: 4, whiteSpace: "pre-wrap" }}>{item.quote}</div>}
      ...
      {editOpen && !isScopedSummary && (
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ color: THEME.textSoft, fontSize: 12, fontWeight: 700 }}>Edit Summary</div>
          ...
          <div style={{ color: THEME.textMuted, fontSize: 10, marginTop: 4 }}>Top Lenses</div>
            ...
            {lensDrafts.map((lens, idx) => (
              <div key={lens.id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  />
                  <label style={{ color: THEME.textMuted, fontSize: 10, display: "flex", alignItems: "center", gap: 4 }}>
                    <input
                      type="checkbox"
                      checked={!lens.hidden}
                ...
                <label style={{ color: THEME.textMuted, fontSize: 10, display: "flex", alignItems: "center", gap: 6 }}>
                  <input
                    type="checkbox"
      ...
      {error && (
        <Centered>
          <div style={{ color: THEME.danger, maxWidth: 420, textAlign: "center" }}>{error}</div>
        </Centered>
      )}
...
function colorForHealth(state: HealthResolution["state"]): string {
  if (state === "good") return "var(--axonmind-health-good, #16a34a)";
  if (state === "watch") return "var(--axonmind-health-watch, #d97706)";
  if (state === "at_risk") return "var(--axonmind-health-risk, #dc2626)";
  return "var(--axonmind-health-unknown, #64748b)";
}
...
function PanelBlock({ title, body }: { title: string; body: string }) {
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ color: THEME.textMuted, fontSize: 10, marginBottom: 4 }}>{title}</div>
      <pre style={{
        margin: 0,
        background: THEME.surface,
        border: `1px solid ${THEME.line}`,
...
function CircleNode({ x, y, size, label, sub, title, bg, border, bold, onClick }: {
  ...
  return (
    ...
    >
      <span style={{
        color: THEME.textStrong, fontSize: bold ? 14 : 12, fontWeight: bold ? 700 : 600, lineHeight: 1.15,
        display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden",
      }}>{label}</span>
      {sub && <span style={{ color: "var(--axonmind-text-sub, rgba(248,250,252,0.7))", fontSize: 10, marginTop: 3 }}>{sub}</span>}
    </div>
  );
}
...
function Centered({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: THEME.textSubtle, fontSize: 14, pointerEvents: "none" }}>
      {children}
    </div>
  );
}
...
const ctrlBtn: React.CSSProperties = {
  padding: "5px 12px", borderRadius: 8, border: `1px solid ${THEME.panelBorder}`,
  background: THEME.line, color: THEME.textMuted, fontSize: 12, cursor: "pointer",
};
